<script lang="ts">
  ...
  import headerBottomLeft from "$lib/assets/header-bottom-left.png";
  ...
  import minesBG from "$lib/assets/mines-bg.png";
  import cavemines from "$lib/assets/cavemines.png";
  ...
  function generateRandomArray() {
    ...
    let decimal = parseFloat((Math.random() * 3 + 1).toFixed(2));
    let currencies = [
      "$", "€", "£", "¥", "₹", "₩", "₽", "₺", "₴", "฿", "₦", "₱", "₪", "₫", 
      "₡", "₲", "₵", "₭", "₮", "₸", "₳", "ƒ", "₣", "Ξ", "⧫", "∂", "₿"
    ];
    let currency = currencies[Math.floor(Math.random() * currencies.length)];
    return [name, number, decimal, currency];
  }
  ...
  onMount(()=>{
    ...
    function onResize() {
      bgElem.style.marginTop = `-${bgElem.offsetHeight}px`;
      if (game.scrollHeight == game.offsetHeight) {
      ...
      <div class="game relative" style="background: radial-gradient(50% 50% at 50% 50%,#151b2e 0,rgba(21,27,46,0) 100%);">
        ...
        <div class="gheader absolute top-[15px] right-[15px] flex gap-2">
          ...
          <button class="bg-[#151b2e] rounded-[8px] text-[#fafafa] text-[13px] p-[5px] px-[11px] flex gap-1 cursor-pointer" aria-label="lok">
            ...
            <span class="mt-[1px]">How to play</span>
          </button>
        </div> 
        ...
        <div class="gamebtns flex flex-col items-center gap-y-3 mx-1.5">
          <div class="status-bar bg-[rgb(21,27,46)] rounded-2xl flex h-[60px] justify-between max-w-[510px] w-full px-4 py-2">
            ...
            <div class="multiply-list w-[18rem] relative pl-2 overflow-hidden flex gap-2 border-l-2 items-center border-[rgb(42,49,69)]">
              <div class="acl-list flex gap-2 items-center transition-all duration-300 ease-in-out" style="transform: translateX(-{multiplyActiveElem?.offsetLeft || (steps == 0 ? 0 : document.querySelector('.acl-list').scrollWidth - 63*5 + 8)}px);">
                {#each data.coef[traps] as x, i }
                  {#if steps!=i}
                  <div class="multiply text-center py-1 px-2 min-w-[55px] h-[32px] rounded-[.5rem] bg-[rgb(29,36,57)] text-white text-[12px] font-bold flex items-center justify-center opacity-30">X{x}</div>
                  {:else}
                  <div class="multiply text-center py-1 px-2 min-w-[55px] h-[32px] rounded-[.5rem] bg-[rgb(29,36,57)] text-white text-[12px] font-bold flex items-center justify-center active" bind:this={multiplyActiveElem}>X{x}</div>
                  {/if}
                {/each}
                {#each {length: 5} as _, i }
                  <div class="multiply text-center py-1 px-2 min-w-[55px] h-[32px] rounded-[.5rem] bg-[rgb(29,36,57)] text-white text-[12px] font-bold flex items-center justify-center opacity-30"></div>
                {/each}
              </div>
            </div>
            {/if}
