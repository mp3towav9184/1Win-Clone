...
<img class="logo mx-auto {hideTitle ? 'pt-4' : 'pt-8'}" src="https://100hp.app/mines/assets/onewin/header-logo-game.png" alt="Logo">
...
<div class="flex items-center justify-center py-4">
  <div class="cells relative" style='background-color: #0a0f1d;background-image: url("{minesCellsBg}");background-position: 50%;background-repeat: no-repeat;background-size: auto;border-radius: 25px;box-shadow: 0 -2px 20px rgba(42,49,69,.4),0 4px 54px rgba(42,49,69,.2);display: grid;grid-template-columns: repeat(5,66px);grid-template-rows: repeat(5,66px);padding: 10px;'>
    {#each list25 as i}
      <button class="cell flex justify-center items-center">
        {#if show && form?.traps?.indexOf(i)==-1}
        ...
        {:else if show && form?.traps?.indexOf(i)!=-1}
...
<div class="traps-selection bg-[rgb(10,15,30)] rounded-lg flex items-center justify-between h-[2.75rem] max-w-[350px] w-full mx-auto py-3 px-4">
  <button aria-label="decrease traps" disabled="{traps==1}" on:click={()=>{traps = traps - 2}} class="disabled:opacity-50 cursor-pointer">
    <svg pointer-events="none" enable-background="new 0 0 15 26" height="20" version="1.1" viewBox="0 0 15 26" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      <polygon fill="#97a3cb" points="12.885,0.58 14.969,2.664 4.133,13.5 14.969,24.336 12.885,26.42 2.049,15.584 -0.035,13.5 "/>
    </svg>
  </button>
  <div class="flex flex-col items-center justify-center">
    <span class="text-[#f3f3f3] text-[16px] font-bold leading-[18px]">{traps}</span>
    <span class="text-[#97a3cb] text-[12px] font-light leading-[12px]">traps</span>
  </div>
  <button aria-label="increase traps" disabled="{traps==7}" on:click={()=>{traps = traps + 2}} class="disabled:opacity-50 cursor-pointer">
    <svg pointer-events="none" style="transform: rotate(180deg)" enable-background="new 0 0 15 26" height="20" version="1.1" viewBox="0 0 15 26" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
      <polygon fill="#97a3cb" points="12.885,0.58 14.969,2.664 4.133,13.5 14.969,24.336 12.885,26.42 2.049,15.584 -0.035,13.5 "/>
    </svg>
  </button>
</div>

<form action="?/getSignal" method="post" use:enhance={()=>{
  btn.disabled = true;
  return async ({ result, update }) => {
    await update();
    btn.disabled = false;
    if (data.isAdmin && form?.traps?.length == 0) return alert('You forgot to START THE GAME FIRST!');
    show = true;
  }
}}>
  <input type="hidden" name="traps" value="{traps}">
  {#if show}
  <button type="button" class="block w-[350px] mx-auto py-3 my-5 rounded-xl text-2xl text-white font-bold cursor-pointer hover:saturate-150 active:scale-75 transition duration-300 ease-out disabled:cursor-not-allowed disabled:opacity-30" style="background: linear-gradient(93.73deg,#108de7,#0855c4);font-family: 'Orbitron', monospace;" on:click={()=>{show = false}}>Reset</button>
  {:else}
  <button bind:this={btn} type="submit" class="block w-[350px] mx-auto py-3 my-5 rounded-xl text-2xl text-white font-bold cursor-pointer hover:saturate-150 active:scale-75 transition duration-300 ease-out disabled:cursor-not-allowed disabled:opacity-30 shadow hover:shadow-amber-600" style="background: linear-gradient(272.98deg,#fdbb4e 2.23%,#f56719 95.05%);font-family: 'Orbitron', monospace;">Get Signal</button>
  {/if}
</form>
...
return async ({ result, update }) => {
  ...
  <button type="submit" disabled={isVerifying} class="text-white bg-cyan-600 shadow shadow-transparent hover:bg-cyan-400 hover:shadow-cyan-700 text-xl font-bold px-3 py-1 rounded-xl outline-2 outline-transparent outline-offset-2 active:scale-75 focus:outline-cyan-400 disabled:opacity-30 transition ease-in-out duration-300">
    {#if isVerifying}
    <span class="block w-[25px] h-[25px] my-0.5 text-transparent mx-auto rounded-full border-b-2 border-white animate-spin">O</span>
    {:else}
    Verify
    {/if}
  </button>
...
<style lang="postcss">
  ...
  .top-banner {
    ...
    padding: 8px 6px;
    border-radius: 10px;
    box-shadow: 0 0 4px #00f0ff, 0 0 8px #00f0ff;
  ...
  .hack-text {
    ...
    text-shadow: 0 0 16px #00f0ff, 0 0 32px #0088ff;
    margin-bottom: 8px;
    user-select: none;
    text-align: center;
  }
